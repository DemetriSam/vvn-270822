        ...
        <form method="POST" action="{{ route('pr_rolls.update', ['pr_roll' => $prRoll]) }} " enctype="multipart/form-data">
            ...
            <div>
                ...
                <x-input 
                    ...
                    type="text" 
                    name="vendor_code" 
                    :value="old('vendor_code') ?? $prRoll->vendor_code" 
                    required 
                    autofocus 
                />
            </div>
            ...
            <div>
                ...
                <x-input 
                    ...
                    type="text" 
                    name="quantity_m2" 
                    :value="old('quantity_m2') ?? $prRoll->quantity_m2" 
                    autofocus 
                />
            </div>
            <div>
                ...
                <select name="pr_cvet_id" id="pr_cvet_id">
                    @foreach ($prCvets as $cvet)
                        <option value="{{ $cvet->id }}">{{ $cvet->name_in_folder }}</option>
                    @endforeach
                </select>
            </div>
            <div>
